<template>
  <div class="pane" style="position: relative;height: 100%">
    <div style="position:relative;width: 400px;border-bottom: #545c64 1px solid;padding-bottom: 5px;height: 100%;font-weight: bold;">
    ...
    submit () {
      if (this.indexing.id === undefined || this.indexing.id === null) {
        ...
        submitIndexingBenchmark(benchmark).then(res => {
          if (res.data === 'success') {
            this.$router.push({path: '/home/benchMarkSuccess'})
          }
        })
      }
    }
